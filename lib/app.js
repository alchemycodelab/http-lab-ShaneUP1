  ...
  socket.on('data', data => {
    ...
    if (request.path === '/' && request.method === 'GET') {
      ...
      socket.end(createResponse({
        body: '<h1>red</h1>', status: '200 OK', contentType: 'html/plain'
      }));
    }

    else if (request.path === '/green' && request.method === 'GET') {
      socket.end(createResponse({
        body: '<h1>green</h1>', status: '200 OK', contentType: 'html/plain'
      }));
    }

    else if (request.path === '/blue' && request.method === 'GET') {
      socket.end(createResponse({
        body: '<h1>blue</h1>', status: '200 OK', contentType: 'html/plain'
      }));
    }
